...
# Diese Felder dürfen von fetch_matches.py niemals geschrieben werden.
FORBIDDEN_MATCH_FIELDS = {
    "market_home_win_odds",
    "market_draw_odds",
    "market_away_odds",
    "market_odds_provider",
    "market_odds_bookmaker_count",
    "market_odds_fetched_at",
    "market_odds_updated_at",
}
...
def map_match(raw: dict[str, Any], source_code: str) -> dict[str, Any] | None:
    ...
    Wichtig:
    Marktquoten werden hier absichtlich NICHT gesetzt.
    Diese Felder gehören ausschließlich fetch_odds.py.
    """
    ...
    row = {
        ...
        # ── Modell-Eingangsdaten: Baseline defaults
        "form_home": [],
        "form_away": [],
        "home_strength": None,
        "away_strength": None,
    ...
    for forbidden_key in FORBIDDEN_MATCH_FIELDS:
        if forbidden_key in row:
            raise RuntimeError(
                f"forbidden field in fetch_matches payload: {forbidden_key}"
            )
...
def run(date_from: str, date_to: str, dry_run: bool) -> None:
    ...
    if failed_codes:
        log.warning(
            "  Status : PARTIAL SUCCESS (%d competition(s) failed, pipeline continues)",
            len(failed_codes),
        )
    else:
